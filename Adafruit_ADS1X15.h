...
protected:
  ...
  uint8_t    _i2c_addr;          /// device address
  i2c_port_t _i2c_channel;       /// device channel
  uint8_t    _m_bitShift;        ///< bit shift amount
  adsGain_t  _m_gain;            ///< ADC gain
  uint16_t   _m_dataRate;        ///< Data rate

public:
  bool begin(uint8_t i2c_addr, i2c_port_t i2c_channel );        //= ADS1X15_ADDRESS
